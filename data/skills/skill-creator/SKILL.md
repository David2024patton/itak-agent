...
In environments without subagents, the core workflow is the same (draft -> test -> review -> improve -> repeat), but some mechanics change. Here's what to adapt:
...
**Description optimization**: This section requires the `claude` CLI tool (specifically `claude -p`) which is only available in certain environments. Skip it if your environment doesn't support it.
