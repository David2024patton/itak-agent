...
1. **Context Gathering**: User provides all relevant context while Antigravity asks clarifying questions
...
**Goal:** Close the gap between what the user knows and what Antigravity knows, enabling smart guidance later.
...
- If images exist without alt-text, explain that when others use an AI assistant to understand the doc, it won't be able to see them. Ask if they want alt-text generated. If so, request they paste each image into chat for descriptive alt-text generation.
...
**If no integrations are detected:** Suggest they can enable connectors in their settings to allow pulling context from messaging apps and document storage directly.
...
Use `create_file` to create an artifact. This gives both Antigravity and the user a scaffold to work from.
...
**If no access to sub-agents (e.g., web interface):**
